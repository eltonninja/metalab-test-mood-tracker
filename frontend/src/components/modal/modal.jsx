...
import { useNewMood } from "@/data-hooks/useMoods";
import { useState } from "react";
import { useRouter } from "next/router";

export default function Modal({ showModal = false, closeModal, scrollToRef }) {
  const [errorMsg, setErrorMsg] = useState("");

  const router = useRouter()

  const { isMutating, trigger } = useNewMood();
  ...
  const updateMood = (newMood) => {
    ...
    trigger(
      { type: newMood },
      {
        ...
        onError() {
          setErrorMsg("Failed to add new mood entry, try again later");
        },
      }
    );
  };
